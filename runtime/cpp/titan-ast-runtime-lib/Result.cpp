...
RichTokenParseErrorData::RichTokenParseErrorData(
    std::list<Token *> *finishedTokens, int startLineNumber, int start,
    int endLineNumber, int end, std::string errorText)
    : finishedTokens(finishedTokens), startLineNumber(startLineNumber),
      start(start), endLineNumber(endLineNumber), end(end),
...
std::string RichTokenParseErrorData::toString() {
  std::stringstream ss;
  ss << "generate token error,error near [" << startLineNumber << "-" << start
     << "," << endLineNumber << "-" << end << "): '" << errorText << "'";
  return ss.str();
}
...
RichAstParseErrorData::RichAstParseErrorData(int startLineNumber, int start,
                                             int endLineNumber, int end,
                                             std::string errorText)
    : startLineNumber(startLineNumber), start(start),
      endLineNumber(endLineNumber), end(end), errorText(std::move(errorText)) {}
RichAstParseErrorData::~RichAstParseErrorData() = default;
std::string RichAstParseErrorData::toString() {
  std::stringstream ss;
  ss << "generate ast error,error near [" << startLineNumber << "-" << start
     << "," << endLineNumber << "-" << end << "): '" << errorText << "'";
  return ss.str();
}
