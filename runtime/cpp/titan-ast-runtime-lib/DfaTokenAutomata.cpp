...
TokensResult *DfaTokenAutomata::buildToken(const std::string *sourceFilePath) {
  ...
  while (true) {
    ...
    if (buildOneTokenMethodResult->type ==
        BuildOneTokenMethodResultType::TOKEN) {
      tokens->push_back(buildOneTokenMethodResult->getTokenData());
      delete buildOneTokenMethodResult;
      continue;
    }
    if (buildOneTokenMethodResult->type ==
        BuildOneTokenMethodResultType::ALL_TEXT_HAS_BEEN_BUILT) {
      tokensResult = TokensResult::generateOkResult(tokens);
      delete buildOneTokenMethodResult;
      break;
    }
    if (buildOneTokenMethodResult->type ==
        BuildOneTokenMethodResultType::TOKEN_PARSE_ERROR) {
    ...
    if (buildOneTokenMethodResult->type ==
        BuildOneTokenMethodResultType::IO_ERROR) {
...
BuildOneTokenMethodResult *DfaTokenAutomata::buildOneToken(
    ByteBufferedInputStream &byteBufferedInputStream) {
  ByteBuffer oneTokenStringBuilder(256);
...
BuildOneTokenMethodResult::~BuildOneTokenMethodResult() {
  switch (type) {
  case BuildOneTokenMethodResultType::TOKEN: // delete data by TokensResult,DfaTokenAutomata::buildToken
