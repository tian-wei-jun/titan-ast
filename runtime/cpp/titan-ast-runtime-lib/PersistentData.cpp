...
GetProductionRulesByInputStreamResult PersistentData::getProductionRulesByInputStream() {
  ...
  for (int indexOfProductionRule = 0;
    ...
    GetSyntaxDfaByInputStreamResult getSyntaxDfaByInputStreamResult = getSyntaxDfaByInputStream();
    if(!getSyntaxDfaByInputStreamResult.isOk){
      return GetProductionRulesByInputStreamResult{false};
    }
    productionRule->reducingDfa = getSyntaxDfaByInputStreamResult.data;
  }
  return GetProductionRulesByInputStreamResult{true,heapProductionRules,countOfProductionRules};
}
...
GetSyntaxDfaByInputStreamResult PersistentData::getSyntaxDfaByInputStream() {
  ...
  return GetSyntaxDfaByInputStreamResult { true,new SyntaxDfa(syntaxDfaStates[0],
...
GetTokenDfaByInputStreamResult PersistentData::getTokenDfaByInputStream() {
  ...
  return GetTokenDfaByInputStreamResult{true,
      new TokenDfa(tokenDfaStates[0], (const TokenDfaState **)tokenDfaStates,
                   sizeOfTokenDfaStates)};
}

GetKeyWordAutomataByInputStreamResult PersistentData::getKeyWordAutomataByInputStream() {
...
GetGrammarsByInputStreamResult PersistentData::getGrammarsByInputStream() {
  ...
  for (int indexOfGrammar = 0; indexOfGrammar < _sizeOfGramamrs;
    ...
    if (type == GrammarType::TERMINAL) {
      ...
      terminalGrammar->lookaheadMatchingMode = LookaheadMatchingMode(readIntResult.data);
    }
...
GetNonterminalFollowMapByInputStreamResult PersistentData::getNonterminalFollowMapByInputStream() {
  ReadIntResult readIntResult = readInt();
  if(!readIntResult.isOk){
  ...
  for (int indexOfNonterminal = 0; indexOfNonterminal < size;
       indexOfNonterminal++) {
    GetGrammarByInputStreamResult getGrammarByInputStreamResult = getGrammarByInputStream();
