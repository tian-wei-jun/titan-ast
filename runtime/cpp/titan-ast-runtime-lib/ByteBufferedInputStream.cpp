...
ByteBufferedInputStream::ByteBufferedInputStream()
    : nextReadIndex(0), eof(-1), nextPos(0), limit(0), markFlag(-1),
      markNextReadIndex(0),limitOfInvalidData(0),
...
ReadResult ByteBufferedInputStream::fillRemainder() {
  ReadResult doReadBufferResult = doReadBuffer(buffer, limit, sizeOfBuffer - limit);
...
ByteBufferedInputStreamInitResult ByteBufferedInputStream::init(const std::string *sourceFilePath) {
  clear();
