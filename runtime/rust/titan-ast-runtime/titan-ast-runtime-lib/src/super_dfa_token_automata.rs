...
impl SuperDfaTokenAutomata {
    ...
    fn build_one_token(
        ...
        let mut one_token_string_builder: ByteBuffer = ByteBuffer {
            is_big_endian: false,
            buffer: Vec::with_capacity(256),
            limit: 0,
        };
        self.start_index_of_token = byte_buffered_input_stream.next_read_index;
        ...
        while ch != byte_buffered_input_stream::EOF {
            ...
            one_token_string_builder.append(ch as u8);
            match next_state {
                Some(next) => {
                    current_state = next;
                }
        ...
        if first_terminal_state.is_none() {
            return BuildOneTokenMethodResult::TokenParseError {
                ...
                end: self.start_index_of_token + one_token_string_builder.len(),
                error_text: one_token_string_builder.to_string(),
            };
        }
        // 重复嗅探更高优先级或贪婪
        let mut length_of_token = one_token_string_builder.len();
        ...
        while ch != byte_buffered_input_stream::EOF {
            ...
            if FaStateType::is_closing_tag(current_state.type_) {
                ...
                if is_higher_priority || is_same_and_greediness {
                    ...
                    length_of_token = one_token_string_builder.len();
                    byte_buffered_input_stream.mark();
                }
        ...
        one_token_string_builder.set_limit(length_of_token);
        //build token
        let terminal_state = heaviest_terminal_state;
        let text_len = one_token_string_builder.len();
        let mut bytes = Vec::with_capacity(text_len);
        for i in 0..text_len {
            bytes.push(one_token_string_builder.buffer[i]);
        }
